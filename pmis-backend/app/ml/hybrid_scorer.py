...
class HybridScorer:
    ...
    def get_recommendations(self, candidate_id=None, top_n=5, candidate_obj=None):
        # PROBLEM 4 FIX — resolve candidate from obj first, then DB
        ...
        # PROBLEM 4 FIX — pass candidate_obj (not raw candidate_id) to content_based
        cb_results = content_based_recommender.recommend(
            candidate_id=None,
            top_n=10000,
            candidate_obj=candidate       # always pass the resolved object
        )
        ...
        for cb in cb_results:
            ...
            # Fetch remaining detail fields from DB (for fields not in CB cache)
            ...
            hybrid_results.append({
                ...
                "required_skills":  cb.get('required_skills', []),   # PROBLEM 4 FIX — expose for verify script
                "location":         location,
